...
#TODO rename in test_wall_air_wall.
...
WALL_SURFACE = BOX_HEIGHT * BOX_DEPTH
...
output_temperature = Output(int(WALL_RESOLUTION / 2), var_name='temperature')
...
wall1 = Component('wall1', brick, THICKNESS, INIT_WALL_TEMPERATURE, FiniteDifferenceTransport(), resolution=WALL_RESOLUTION, surface=WALL_SURFACE, observer=observer)
...
air_component = Component('room', air, BOX_WIDTH, INIT_AIR_TEMPERATURE, FiniteDifferenceTransport(), resolution=BOX_RESOLUTION, surface=WALL_SURFACE, observer=observer)
...
# Box object has y axis composed of 3 points (2 ghost points and a physics point), so we choose index 1 to obtain the physics point.
box_component = Component('room', air, BOX_WIDTH, INIT_AIR_TEMPERATURE, FiniteVolume(), resolution=BOX_RESOLUTION, surface=WALL_SURFACE, observer=observer)
...
def test_box():
    neighbours = {'in': air_exterior, 'ext': box_component}
    wall1.set_neighbours(neighbours)
    neighbours = {'in': box_component, 'ext': air_exterior}
    wall2.set_neighbours(neighbours)
    neighbours = {'in': wall1, 'ext': wall2}
    box_component.set_neighbours(neighbours)
    component_to_solve_list = [wall1, box_component, wall2]
    solver = Solver(component_to_solve_list, DT, TIME_END)
    solver.run()
    solver.post()
